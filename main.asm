...
.equ count = 0x3b04				; assign a 16-bit value to symbol "count"

delay_long:
	ldi r30, low(count)	  	; r31:r30  <-- load a 16-bit value into counter register for outer loop
	ldi r31, high(count);
d1:
	ldi   r29, 0x5f		    	; r29 <-- load a 8-bit value into counter register for inner loop
d2:
	nop											; no operation
	nop
	nop
	...
	brne d1	
	nop								; branch to d1 if result is not "0"
	ret											; return
